...
"""
version = 'BrainHunt EX 3.22/18.02.23'
...
if __name__ == "__main__":
    ...
    raw:bool = False
    incdec:int = 1
    th, bf_dir, rescan_dir, telegram_enable, id, desc, save, minout, raw, incdec  = createParser()
                    ...
                    if res[0] == 'btc' and cbtc:
                        for BF in list_btc:
                            if BF.check(res[3]):
                                ...
                                PVK = normalize(res[2])
                                print(f'\n{color.green}[F bloom] FOUND {date_str()} word:{WORD} PVK:{PVK} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                save_file('found-bloom',f'[F bloom] FOUND {date_str()} word:{WORD} PVK:{PVK} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                if crescan:
                                    rez = rescan(res[3], 'btc', rescan_dir)
                                    if rez == None: crescan = False
                                    elif rez == True:
                                        print(f'\n{color.green}[F True] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                        save_file('found',f'[F True] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                        if telegram_enable:
                                            send_telegram(f'[F True] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}', telegram_channel_id, telegram_token)
                                    else:
                                        print(f'\n{color.green}[F False] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                        save_file('found-false',f'[F False] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                else:
                                    print(f'\n{color.green}[F] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                    save_file('found',f'[F] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                    if telegram_enable:
                                        send_telegram(f'[F] FOUND {date_str()} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}', telegram_channel_id, telegram_token) 
                            co += 1
                    ...
                    if res[0] == 'eth' and ceth:
                        for BF in list_eth:
                            if BF.check(res[3]):
                                WORD = normalize(res[1])
                                PVK = normalize(res[2])
                                print(f'\n{color.green}[F bloom] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{PVK} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                save_file('found-bloom',f'[F bloom] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{PVK} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                if crescan:
                                    rez = rescan(res[3], 'btc', rescan_dir)
                                    if rez == None: crescan = False
                                    elif rez == True:
                                        print(f'\n{color.green}[F True] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                        save_file('found',f'[F True] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                        if telegram_enable:
                                            send_telegram(f'[F True] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}', telegram_channel_id, telegram_token)
                                    else:
                                        print(f'\n{color.green}[F false] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                        save_file('found-false',f'[F false] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                else:
                                    print(f'\n{color.green}[F] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}\n')
                                    save_file('found',f'[F] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}')
                                    if telegram_enable:
                                        send_telegram(f'[F] FOUND {date_str()} ETH:0x{res[3]} word:{WORD} PVK:{(res[2])} Algo:{res[4]} ID:{id} desc:{desc} {in_file}', telegram_channel_id, telegram_token)
                            co += 1
            try:
