...
if __name__ == '__main__':
    ...
    maxLength = input("до скольки слов генерировать ? (например указав 5 будут сгенерированы все комбинации от 1 слова до 5) > ")
    if int(maxLength) == 0: exit(1)
    for _ in range(int(maxLength) - 1):
        data.append(' ')
    ...
    for i in range(int(maxLength)):
        pwd = product(data, repeat = int(i+1))
        for i in pwd:
            co += 1
            password = "".join(i)
